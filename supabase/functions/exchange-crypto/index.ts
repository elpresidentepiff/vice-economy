...
const allowedCurrencies = new Set(['cash_clean', 'sim_usdt', 'sim_usdc'])
...
function parseCurrency(value: unknown): string | null {
  ...
  return allowedCurrencies.has(trimmed) ? trimmed : null
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const fromCurrency = parseCurrency(body.from_currency)
    const toCurrency = parseCurrency(body.to_currency)
    const amount = parseAmount(body.amount)

    if (!fromCurrency || !toCurrency || fromCurrency === toCurrency) {
      return jsonResponse({ success: false, error: 'invalid_exchange_pair' }, 400)
    }
    ...
    const { data: result, error: rpcError } = await caller.rpc('exchange_currency', {
