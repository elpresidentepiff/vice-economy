...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await req.json().catch(() => ({})) as BribeBody
    if (typeof body.district_id !== 'string' || body.district_id.trim().length === 0) {
      return jsonResponse({ success: false, error: 'invalid_district_id' }, 400)
    }
    ...
    const { data: result, error: rpcError } = await caller.rpc('bribe_police', {
      p_player_id: authData.user.id,
      p_district_id: body.district_id.trim(),
      p_amount: amount,
    })
